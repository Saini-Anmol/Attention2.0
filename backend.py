...
def handle_nvd_query(query, api_key):
    ...
    **RESTORED VERSION**: This function now uses the AI to intelligently parse the NVD data.
    ...
    try:
        ...
        prompt = f"You are a precise data extraction tool. Based *only* on the provided JSON context, answer the user's question. If the data for the user's question does not exist, say 'N/A'. User question: '{query}'. JSON context: ```json {context} ```"
